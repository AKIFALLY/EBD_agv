# Docker 容器管理工具 Prompt

## 🎯 適用場景
- Docker 容器的啟動、停止、重啟管理
- 容器狀態監控和健康檢查
- AGV 和 AGVC 雙環境容器管理
- 容器內環境快速進入和命令執行
- 系統整體容器狀態診斷

## 📋 核心工具概述
RosAGV 專用 Docker 管理工具集，提供統一的容器操作介面，支援 AGV 車載系統和 AGVC 管理系統的雙環境管理。

## 🔧 工具使用指導

### 🚀 超簡單使用方式 (推薦)
```bash
# 使用統一工具入口 (最簡單)
r containers-status      # 檢查所有容器狀態
r agvc-start            # 啟動 AGVC 系統
r agvc-stop             # 停止 AGVC 系統
r agv-start             # 啟動 AGV 容器
r agv-stop              # 停止 AGV 容器
```

### 載入專業工具集 (進階使用)
```bash
# 載入所有Docker管理工具
source scripts/docker-tools/docker-tools.sh
show_docker_tools_help
```

### AGV 容器管理 (專業工具集)
```bash
agv_start         # 啟動 AGV 容器
agv_stop          # 停止 AGV 容器
agv_enter         # 進入 AGV 容器 (自動載入 agv_source)
agv_logs          # 查看 AGV 日誌
agv_health        # AGV 健康檢查
agv_status        # AGV 容器詳細狀態
```

### AGVC 容器管理 (專業工具集)
```bash
agvc_start        # 啟動 AGVC 系統 (所有服務)
agvc_stop         # 停止 AGVC 系統
agvc_enter        # 進入 AGVC 容器 (自動載入 agvc_source)
agvc_logs         # 查看 AGVC 日誌
agvc_health       # AGVC 健康檢查
agvc_services     # 檢查所有 AGVC 服務狀態
agvc_status       # AGVC 容器詳細狀態
```

### 系統整體操作 (專業工具集)
```bash
all_start         # 啟動所有系統 (AGVC + AGV)
all_stop          # 停止所有系統
all_restart       # 重啟所有系統
all_status        # 查看所有容器狀態
all_health        # 智能健康狀態判斷
all_ports         # 檢查端口狀態
all_resources     # 檢查資源使用情況
all_network       # 檢查網路配置
all_summary       # 系統狀態摘要
```

### 快速命令執行 (專業工具集)
```bash
quick_agv "ros2 node list"    # 在 AGV 容器執行 ROS 2 指令
quick_agvc "check_agvc_status" # 在 AGVC 容器執行系統檢查
```

## 🎯 使用策略

### 開發場景選擇
| 開發任務 | 推薦操作 | 自動載入環境 |
|---------|---------|-------------|
| AGV狀態機開發 | `agv_enter` | agv_source (9個工作空間) |
| Web API開發 | `agvc_enter` | agvc_source (11個工作空間) |
| 系統整合測試 | `all_start` + 分別進入 | 根據容器自動選擇 |
| 問題診斷 | `all_health` | N/A |

### 工作流程最佳實踐
```bash
# 1. 系統啟動檢查
all_status                # 檢查容器運行狀態
all_health                # 全面健康檢查

# 2. 開發環境準備
agv_enter                 # 或 agvc_enter
# 自動執行: source /app/setup.bash && agv_source

# 3. 系統狀態驗證
check_system_status       # 容器內環境檢查
check_zenoh_status        # 通訊狀態確認
```

## 🔧 進階功能

### 組合操作範例
```bash
# 完整系統重啟和檢查
all_stop && sleep 5 && all_start && sleep 10 && all_health

# 快速診斷問題
all_status && all_ports && quick_agv "ros2 node list" && quick_agvc "check_agvc_status"

# 開發環境快速設置
agvc_start && agvc_enter
# 在容器內自動執行環境載入
```

### 狀態監控
```bash
# 容器狀態詳情
scripts/docker-tools/container-status.sh all --verbose

# 端口衝突檢查
scripts/docker-tools/container-status.sh ports --conflicts

# 資源使用監控
scripts/docker-tools/container-status.sh resources --monitoring
```

## 🚨 故障排除

### 常見問題診斷
| 問題症狀 | 診斷指令 | 解決方案 |
|---------|---------|---------|
| 容器無法啟動 | `all_status` | 檢查端口衝突、配置檔案 |
| 無法進入容器 | `agv_health`/`agvc_health` | 確認容器運行狀態 |
| 環境載入失敗 | 容器內 `check_system_status` | 檢查工作空間建置狀態 |
| 通訊中斷 | `all_ports` + `check_zenoh_status` | 檢查Zenoh Router狀態 |

### 緊急操作
```bash
# 強制重啟所有服務
docker compose -f docker-compose.agvc.yml down --remove-orphans
docker compose -f docker-compose.yml down --remove-orphans
all_start

# 清理異常容器
docker system prune -f
docker volume prune -f
```

## 🔍 除錯檢查清單

### 啟動前檢查
- [ ] 確認Docker服務運行
- [ ] 檢查端口7447, 8000-8002, 5432是否被佔用
- [ ] 驗證配置檔案存在且格式正確

### 運行中檢查
- [ ] `all_status` 確認所有容器運行
- [ ] `all_health` 檢查系統健康狀態
- [ ] 容器內 `check_zenoh_status` 確認通訊正常
- [ ] 容器內 `check_system_status` 確認工作空間狀態

### 開發環境檢查
- [ ] 進入容器後環境自動載入
- [ ] `ros2 node list` 顯示預期節點
- [ ] 相關服務（資料庫、Web）正常回應