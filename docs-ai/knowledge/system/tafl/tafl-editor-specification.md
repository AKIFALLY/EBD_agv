# TAFL Editor 規格書 (Specification Document)

## 📋 文件資訊
- **版本**: v3.1
- **更新日期**: 2025-09-03
- **實作位置**: `/home/ct/RosAGV/app/web_api_ws/src/agvcui/agvcui/static/js/tafl-editor.js`
- **套件歸屬**: agvcui (車隊管理界面系統)
- **服務端口**: 8001 (AGVCUI 管理界面)
- **檔案大小**: 2606 行 (單體架構，計劃模組化)
- **執行模式**: Real Mode (PostgreSQL) / Simulation Mode

## 🎯 系統概述

### 產品定義
**TAFL Editor** 是一個專業的視覺化流程編輯器，用於創建和編輯 TAFL (Task Automation Flow Language) 格式的自動化流程。提供拖放式介面、即時驗證、視覺化預覽等企業級功能。

### 核心定位
- **所屬系統**: agvcui 車隊管理界面的一部分
- **使用者**: 系統管理員、流程設計師
- **用途**: 設計 AGV 任務自動化流程、WCS 工作流程
- **輸出格式**: TAFL v1.1 YAML 檔案

### 技術架構
- **前端框架**: Vanilla JavaScript ES6
- **CSS 框架**: Bulma 1.0.4
- **圖標庫**: Font Awesome 5
- **編輯器**: CodeMirror (YAML 模式)
- **儲存系統**: agvcui miniStore (計劃整合)
- **後端 API**: FastAPI `/tafl/` 端點群組
- **執行引擎**: tafl_editor_direct.py (增強版)
- **資料庫橋接**: TAFLDatabaseBridge (PostgreSQL)

## 📐 功能規格

### 1. 流程結構支援 (TAFL v1.1)

#### 六段式結構 (6-Segment Structure)
```yaml
# TAFL v1.1 完整結構
metadata:       # 流程元資料
settings:       # 執行設定 (Phase 0)
preload:        # 資料預載 (Phase 1)
rules:          # 業務規則 (Phase 2)
variables:      # 變數定義 (Phase 3)
flow:           # 主要流程 (Phase 4)
```

#### 四階段執行模型 (4-Phase Execution Model)
1. **Phase 0**: Settings - 設定執行參數
2. **Phase 1**: Preload - 預載必要資料
3. **Phase 2**: Rules - 驗證業務規則
4. **Phase 3**: Variables - 初始化變數
5. **Phase 4**: Flow - 執行主要流程

### 2. 視覺化編輯功能

#### 拖放操作 (Drag & Drop)
- **動詞卡片拖放**: 從左側面板拖動動詞到畫布
- **卡片重新排序**: 拖動卡片改變執行順序
- **巢狀結構支援**: 支援 if/for 等控制結構的巢狀拖放
- **視覺回饋**: 拖動時顯示插入位置指示器
- **自動捲動**: 拖動到邊緣時自動捲動畫布

#### 卡片顯示 (Card Display)
- **圖標系統**: 每個動詞有對應的 Font Awesome 圖標
- **顏色編碼**: 不同類別動詞使用不同顏色
- **狀態指示**: 選中、錯誤、警告等狀態視覺化
- **巢狀縮排**: 控制結構內的卡片自動縮排顯示
- **連線渲染**: 顯示卡片間的執行流程連線

### 3. 動詞支援 (Verb Support)

#### 支援的 TAFL 動詞 (10個)
基於 `/app/web_api_ws/src/agvcui/agvcui/routers/tafl_editor.py` 的實際實作：

```python
# 實際支援的 TAFL v1.1.2 動詞
TAFL_VERBS = {
    "query": {
        "name": "Query",
        "description": "Query data from database or system",
        "color": "is-info",
        "icon": "fas fa-database",
        "params": ["target", "where", "limit", "as"]
    },
    "check": {
        "name": "Check",
        "description": "Check conditions or status",
        "color": "is-success",
        "icon": "fas fa-check-circle",
        "params": ["condition", "as"]  # v1.1.3: 'as' is required
    },
    "create": {
        "name": "Create",
        "description": "Create new resources or records",
        "color": "is-primary",
        "icon": "fas fa-plus-circle",
        "params": ["target", "with", "as"]
    },
    "update": {
        "name": "Update",
        "description": "Update existing data",
        "color": "is-warning",
        "icon": "fas fa-edit",
        "params": ["target", "where", "set"]
    },
    "if": {
        "name": "If",
        "description": "Conditional execution",
        "color": "is-danger",
        "icon": "fas fa-question-circle",
        "params": ["condition", "then", "else"]
    },
    "for": {
        "name": "For",
        "description": "Loop through collection",
        "color": "is-purple",
        "icon": "fas fa-redo",
        "params": ["in", "as", "do", "filter"]
    },
    "switch": {
        "name": "Switch",
        "description": "Multi-branch conditional",
        "color": "is-orange",
        "icon": "fas fa-code-branch",
        "params": ["expression", "cases"]
    },
    "set": {
        "name": "Set",
        "description": "Set variable values",
        "color": "is-dark",
        "icon": "fas fa-equals",
        "params": []  # Dictionary format only in v1.1.2
    },
    "stop": {
        "name": "Stop",
        "description": "Stop flow execution",
        "color": "is-danger-dark",
        "icon": "fas fa-stop-circle",
        "params": ["reason", "condition"]
    },
    "notify": {
        "name": "Notify",
        "description": "Send notifications",
        "color": "is-link",
        "icon": "fas fa-bell",
        "params": ["level", "message", "recipients", "details"]
    }
}
```

#### ⚠️ 不支援的動詞
以下動詞在 TAFL v1.1.2 中**不存在**：
- ❌ delete (刪除)
- ❌ while (條件迴圈)
- ❌ break/continue (迴圈控制)
- ❌ call/return (函數呼叫)
- ❌ wait (等待)
- ❌ get (取得，使用 query 替代)
- ❌ exec/execute (執行外部命令)
- ❌ publish/subscribe (發布訂閱)
- ❌ api/mqtt/websocket (外部通訊)
- ❌ log/alert/validate/transform (工具函數)

### 4. 側邊面板功能

#### 左側面板 - 動詞庫
- **動詞列表**: 10 個支援的 TAFL 動詞
- **動詞卡片**: 可拖動的動詞卡片列表
- **搜尋功能**: 快速搜尋動詞（計劃功能）
- **說明提示**: 滑鼠懸停顯示動詞說明

#### 右側面板 - 屬性編輯
- **Metadata 標籤**: 編輯流程元資料
- **Variables 標籤**: 管理流程變數
- **Preload 標籤**: 配置預載資料
- **Rules 標籤**: 定義業務規則
- **Settings 標籤**: 執行設定
- **Properties 標籤**: 編輯選中卡片屬性
- **YAML 標籤**: 檢視/編輯原始 YAML

### 5. 編輯器功能

#### 基本編輯
- **新增卡片**: 拖放或雙擊動詞
- **刪除卡片**: Delete 鍵或刪除按鈕
- **複製卡片**: Ctrl+D 或複製按鈕
- **選擇卡片**: 單擊選中，顯示屬性
- **批量操作**: 多選支援（計劃功能）

#### 屬性編輯
- **動態表單**: 根據動詞定義生成參數表單
- **類型驗證**: 即時驗證參數類型
- **必填檢查**: 標示必填參數
- **智能提示**: 參數值建議和自動完成

#### YAML 編輯
- **CodeMirror 整合**: 語法高亮、自動縮排
- **雙向同步**: YAML 與視覺化即時同步
- **錯誤提示**: YAML 語法錯誤即時顯示
- **格式化**: 自動格式化 YAML 內容

### 6. 資料管理

#### 變數管理 (Variables)
```javascript
variables = {
    "user_id": 123,
    "status": "active",
    "items": ["item1", "item2"],
    "config": { "timeout": 30 }
}
```
- **CRUD 操作**: 新增、編輯、刪除變數
- **類型支援**: string、number、boolean、array、object
- **驗證規則**: 變數名稱格式驗證
- **使用追蹤**: 顯示變數使用位置

#### 預載資料 (Preload)
```javascript
preload = {
    "users": {
        query: {
            type: "database",
            target: "users",
            select: ["id", "name", "role"],
            where: "status = 'active'"
        }
    }
}
```
- **資料來源**: database、api、file、constant
- **查詢建構器**: 視覺化查詢配置
- **快取設定**: 資料快取策略
- **重新整理**: 手動重新載入資料

#### 業務規則 (Rules)
```javascript
rules = {
    "max_retry": 3,
    "timeout": 300,
    "require_approval": true,
    "validation_pattern": "^[A-Z]{3}\\d{4}$"
}
```
- **規則類型**: validation、constraint、calculation、condition
- **啟用控制**: 規則啟用/停用開關
- **優先順序**: 規則執行順序設定
- **描述說明**: 規則用途說明文字

### 7. 檔案操作

#### 載入流程
- **流程列表**: 從 `/tafl/flows` API 獲取
- **載入對話框**: 選擇並載入現有流程
- **格式驗證**: 載入時驗證 TAFL 格式
- **錯誤處理**: 顯示載入錯誤詳情

#### 儲存流程
- **儲存對話框**: 輸入流程 ID 和名稱
- **覆寫確認**: 覆寫現有流程時確認
- **格式轉換**: 視覺化轉換為 YAML
- **成功通知**: 儲存成功提示

#### 匯出/匯入
- **匯出 YAML**: 下載為 .yaml 檔案
- **匯入 YAML**: 上傳並載入 .yaml 檔案
- **格式轉換**: 支援舊版格式升級

### 8. 驗證功能

#### 即時驗證
- **參數驗證**: 檢查必填參數
- **類型檢查**: 驗證參數類型
- **引用檢查**: 驗證變數引用
- **語法檢查**: YAML 語法驗證

#### 流程驗證
- **結構驗證**: 檢查流程結構完整性
- **邏輯驗證**: 檢查控制流程邏輯
- **依賴驗證**: 檢查資料依賴關係
- **執行路徑**: 分析可能的執行路徑

### 9. 執行功能

#### 測試執行
- **乾跑模式**: 不實際執行，僅驗證
- **單步執行**: 逐步執行並顯示狀態
- **斷點設定**: 在特定卡片設定斷點
- **變數監看**: 即時顯示變數值

#### 執行監控
- **進度顯示**: 顯示執行進度
- **日誌輸出**: 即時顯示執行日誌
- **錯誤追蹤**: 顯示錯誤位置和詳情
- **性能分析**: 顯示各步驟執行時間

## 🔌 API 整合

### 後端 API 端點
```python
# FastAPI 路由定義
GET    /tafl/verbs           # 獲取動詞定義
GET    /tafl/flows           # 獲取流程列表
GET    /tafl/flows/{flow_id} # 獲取特定流程
POST   /tafl/flows           # 創建新流程
PUT    /tafl/flows/{flow_id} # 更新流程
DELETE /tafl/flows/{flow_id} # 刪除流程
POST   /tafl/validate        # 驗證流程
POST   /tafl/execute         # 執行流程
POST   /tafl/convert-to-yaml # JSON 轉 YAML
POST   /tafl/import-yaml     # YAML 匯入
```

### API 實作狀態 (2025-09-02 驗證)
| API 端點 | 實際功能 | 實作狀態 | 備註 |
|---------|---------|----------|------|
| `/tafl/verbs` | 返回動詞定義 | ✅ 完整實作 | 10個核心動詞 |
| `/tafl/flows` | 列出所有流程 | ✅ 完整實作 | 從 `/app/config/tafl/flows/` 讀取 |
| `/tafl/flows/{id}` | 獲取特定流程 | ✅ 完整實作 | 支援 v1.1 格式 |
| `/tafl/flows` (POST) | 保存流程 | ✅ 完整實作 | 自動清理 card IDs |
| `/tafl/flows/{id}` (DELETE) | 刪除流程 | ✅ 完整實作 | 刪除 YAML 檔案 |
| `/tafl/validate` | 驗證流程 | ✅ 完整實作 | 支援 v1.1 六段式驗證 |
| `/tafl/execute` | 執行流程 | ⚠️ 模擬執行 | 僅 dry run，未實際執行 |
| `/tafl/convert-to-yaml` | 格式轉換 | ✅ 完整實作 | JSON → YAML |
| `/tafl/import-yaml` | 格式匯入 | ✅ 完整實作 | YAML → JSON |

**重要說明**：
- 流程儲存位置：`/app/config/tafl/flows/` (容器內路徑)
- 執行功能：目前僅模擬執行，實際執行需整合 TAFL_WCS 或其他執行引擎
- ID 清理：`clean_card_ids()` 函數會自動移除編輯器生成的暫時 IDs

### 資料格式
```typescript
// TypeScript 介面定義（參考用）
interface TAFLFlow {
    metadata: {
        id: string;
        name: string;
        version: string;
        description: string;
        created_at?: string;
        updated_at?: string;
    };
    settings?: {
        timeout: number;
        max_retries: number;
        retry_on_failure: boolean;
    };
    preload?: Record<string, PreloadConfig>;
    rules?: Record<string, any>;
    variables?: Record<string, any>;
    flow: FlowStep[];
}

interface FlowStep {
    id: string;
    [verb: string]: any;  // 動詞參數
    comment?: string;
    skip_if?: string;
    store_as?: string;
}
```

## 💾 資料儲存

### localStorage 結構（現行）
```javascript
// 當前使用原生 localStorage
localStorage.setItem('tafl_current_flow', JSON.stringify(flow));
localStorage.setItem('tafl_recent_flows', JSON.stringify(recentList));
localStorage.setItem('tafl_editor_preferences', JSON.stringify(prefs));
```

### miniStore 整合（計劃）
```javascript
// 計劃整合 agvcui 的 miniStore 系統
import { createStore } from '../store/miniStore.js';

const taflStore = createStore('taflEditor', {
    currentFlow: null,
    recentFlows: [],
    preferences: {},
    isDirty: false
});
```

## 🎨 UI/UX 規格

### 視覺設計
- **主題**: Bulma 預設主題 + 自訂擴展
- **配色方案**: 
  - Flow: 藍色 (is-info)
  - Control: 黃色 (is-warning)
  - Data: 綠色 (is-success)
  - Task: 主色 (is-primary)
  - Error: 紅色 (is-danger)
- **字體**: 系統預設字體
- **圖標**: Font Awesome 5 圖標集

### 響應式設計
- **最小寬度**: 1024px（不支援行動裝置）
- **建議解析度**: 1920x1080
- **瀏覽器支援**: Chrome、Firefox、Edge（現代瀏覽器）

### 互動設計
- **拖放回饋**: 拖動時半透明顯示
- **懸停效果**: 顯示工具提示
- **選中狀態**: 藍色邊框高亮
- **錯誤狀態**: 紅色邊框和圖標
- **載入狀態**: 載入動畫和遮罩

## 🔄 模組化計劃

### 計劃模組結構
```
tafl-editor/
├── core/
│   ├── tafl-editor-core.js      # 主控制器
│   ├── tafl-editor-state.js     # 狀態管理（miniStore）
│   └── tafl-editor-config.js    # 配置管理
├── ui/
│   ├── tafl-editor-ui.js        # UI 更新
│   ├── tafl-editor-cards.js     # 卡片渲染
│   ├── tafl-editor-panels.js    # 面板管理
│   └── tafl-editor-modals.js    # 對話框
├── interactions/
│   ├── tafl-editor-drag-drop.js # 拖放功能
│   ├── tafl-editor-events.js    # 事件處理
│   └── tafl-editor-selection.js # 選擇管理
├── data/
│   ├── tafl-editor-flow.js      # 流程管理
│   ├── tafl-editor-variables.js # 變數管理
│   ├── tafl-editor-preload.js   # 預載管理
│   └── tafl-editor-rules.js     # 規則管理
├── api/
│   ├── tafl-editor-api.js       # API 通訊
│   └── tafl-editor-storage.js   # 儲存管理
└── utils/
    ├── tafl-editor-yaml.js      # YAML 處理
    ├── tafl-editor-validation.js # 驗證邏輯
    └── tafl-editor-helpers.js   # 輔助函數
```

### 模組化優勢
- **程式碼組織**: 從 2606 行單檔案拆分為 19 個模組
- **維護性**: 每個模組專注單一職責
- **可測試性**: 模組化便於單元測試
- **miniStore 整合**: 統一狀態管理和自動持久化
- **程式碼縮減**: 透過 miniStore 減少約 170 行程式碼

## 📊 效能規格

### 效能目標
- **載入時間**: < 2 秒（包含動詞定義載入）
- **拖放響應**: < 50ms
- **流程驗證**: < 500ms（100 個步驟）
- **YAML 同步**: < 100ms
- **最大流程**: 支援 500+ 步驟

### 記憶體管理
- **最大流程大小**: 10MB
- **撤銷歷史**: 保留最近 50 個操作
- **快取策略**: LRU 快取最近 10 個流程

## 🐛 已知問題與限制

### 當前限制
1. **單體架構**: 2606 行單一檔案，維護困難
2. **無撤銷功能**: 尚未實作 Undo/Redo
3. **無批量操作**: 不支援多選和批量編輯
4. **無搜尋功能**: 動詞庫無搜尋功能
5. **無版本控制**: 流程無版本歷史記錄

### 已知問題
1. **Preload/Rules 顯示**: 載入流程時需手動呼叫 refreshPreloadUI() 和 refreshRulesUI()
2. **拖放閃爍**: 某些情況下拖放會造成畫面閃爍
3. **記憶體洩漏**: 長時間使用可能有輕微記憶體洩漏

## 🔮 未來發展

### 短期計劃（1-3 個月）
1. **模組化重構**: 執行 19 個模組的拆分計劃
2. **miniStore 整合**: 使用 agvcui 的 miniStore 系統
3. **撤銷/重做**: 實作 Undo/Redo 功能
4. **搜尋功能**: 新增動詞搜尋和流程內搜尋

### 中期計劃（3-6 個月）
1. **協作功能**: 多人同時編輯支援
2. **版本控制**: 流程版本歷史和比較
3. **模板系統**: 預設流程模板庫
4. **除錯工具**: 增強的除錯和分析功能

### 長期願景（6+ 個月）
1. **AI 輔助**: AI 建議和自動完成
2. **視覺化增強**: 更豐富的視覺化效果
3. **外掛系統**: 支援自訂動詞和擴展
4. **行動支援**: 響應式設計支援平板

## 📚 相關文件

### 開發文件
- **模組化計劃**: docs-ai/knowledge/system/tafl-editor-modularization/
- **miniStore 整合狀態**: docs-ai/knowledge/system/tafl-editor-modularization/ministore-integration-status.md
- **TAFL 語言規格**: docs-ai/knowledge/system/tafl/tafl-language-specification.md

### 實作位置
- **主程式**: `/home/ct/RosAGV/app/web_api_ws/src/agvcui/agvcui/static/js/tafl-editor.js`
- **樣式表**: `/home/ct/RosAGV/app/web_api_ws/src/agvcui/agvcui/static/css/tafl-editor.css`
- **後端 API**: `/home/ct/RosAGV/app/web_api_ws/src/agvcui/agvcui/routers/tafl.py`

### 測試檔案
- **整合測試**: `/home/ct/RosAGV/agents/test_tafl_editor_*.py`
- **UI 測試**: `/home/ct/RosAGV/agents/test_flow_designer_*.html`

## 🔄 版本歷史

### v3.0 (2025-01)
- 完整重寫，專業拖放體驗
- 支援 TAFL v1.1 六段式結構
- 新增 Preload、Rules、Settings 支援
- 改進視覺化和使用者體驗

### v2.0 (2024-12)
- 支援巢狀結構
- 新增 YAML 編輯器
- 改進拖放功能

### v1.0 (2024-11)
- 初始版本
- 基本拖放功能
- 簡單流程編輯

---

**文件維護說明**: 
- 本規格書應與實際實作同步更新
- 任何功能變更都應更新對應章節
- 模組化實作時需參考本規格書確認功能完整性