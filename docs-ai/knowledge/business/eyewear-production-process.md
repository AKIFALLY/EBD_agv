...
### 射出機作業區配置
```
單個作業員 (OP) 工作區域配置（🆕 2025-09 更新）：
├── 工作區（workspace）：可配置多個位置
│   ├── 位置1：人工搬運空 Rack 的放置處
│   ├── 位置2：暫存裝載中的 Rack
│   └── 位置3：備用工作區位置
├── 停車格（parking_space）：1個固定位置
│   └── KUKA AGV 專用，僅供派車使用
└── 作業員在這些位置間管理 Rack 裝載

重要概念：
- 每個作業員擁有獨立的工作區（可配置 1-6 個位置）
- 工作區：用於暫存和裝載 Rack，系統自動分配可用位置
- 停車格：專門留給 KUKA AGV，派車時 Rack 從工作區移到停車格
- Rack 本身是一台可推動的料架車
```
...
### OPUI 完整操作流程
```
作業員完整操作流程（🆕 2025-09 工作區配置更新）
1. 【手動加入 Rack】當需要新空rack時：
   ├── 作業員從倉儲區手動搬運空rack到工作區
   ├── 在OPUI首頁點擊對應側的「加入料架」按鈕（左側或右側）
   ├── 從彈出視窗的下拉選單中選擇 Rack 編號（如 "101"）
   ├── 🆕 系統自動分配到第一個可用的工作區位置
   ├── 工作區滿時顯示「工作區已滿，請等待料架派送完成」
   └── 成功時顯示「已將料架 101 加入到工作區」確認訊息
...
4. 【派滿車】當rack裝滿後：
   ├── 按「派車」按鈕產生任務請求
   ├── 🆕 系統自動將 Rack 從工作區移到停車格
   ├── WCS系統根據房間製程需求決定調度
   └── KUKA AGV 從停車格取走 Rack 運送到目的地
...
#### 核心操作功能
1. **叫車功能** (Call Empty Car) - 已停用
   - 按鈕：`data-call-empty="left/right"`
   - 用途：~~請求 KUKA AGV 送空 rack 到停車格~~ （已停用，改由人工手動搬運）
   - 狀態：功能保留但已停用，不會產生實際任務
...
### 已確認理解的環節
1. **射出機生產** → 眼鏡產品射出
2. **手動搬運空rack** → 作業員手動搬運空rack到工作區A/B
3. **OPUI加入rack** → 使用首頁「加入料架」按鈕登記空rack
4. **人工作業** → 眼鏡掛載到carrier，carrier安裝到rack
5. **OPUI派滿車** → 設定rack內容物(產品、數量)和目標房間
6. **派車請求** → KUKA AGV將滿載rack從工作區搬到停車格，再由WCS調度搬運
...
#### 🔴 第1級：AGV旋轉檢查 (Priority: 100) - 雙旋轉點流程
```
Rack 180度轉向流程（房間入口 + 房間出口）

**房間入口旋轉** (A面空B面工作) ⚠️ 已開發未啟用
房間X入口 → 中間點 → 房間X入口
├── 觸發條件：A面載具已取走（0個），B面有待作業載具
├── 用途：讓B面的載具能被Cargo AGV卸載到入口傳送箱
├── 3節點移動方式實現
├── 流程：rack_rotation_room_inlet_aempty_bwork.yaml (enabled: false)
└── Work ID: 220001 (kuka-移動貨架)

**房間出口旋轉** (A面滿B面空) ✅ 已啟用
房間X出口 → 中間點 → 房間X出口
├── 觸發條件：A面已滿（16個載具），B面為空（0個），房間內還有載具待處理
...
#### 🟠 第2級：滿料架到人工收料區 (Priority: 80) ❌ 待實作
```
製程完成後的 Rack 處理
房間X出口 → 人工收料區 (21-22)
├── 製程完成的滿料架搬運
├── 人工取出製程完成的 Carrier
├── OPUI-HMI「移出系統」(location_id = null)
├── 人工收料區後續全由人工處理
└── Work ID: 220001 (kuka-移動貨架)
```

#### 🟡 第3級：系統準備區到房間 (Priority: 60) ✅ 已實作
```
系統投料流程
系統準備區 (11-18) → 房間X入口
├── OPUI 派車後的 Rack 調度
...
├── 一次只處理一個 Rack，避免衝突
├── 房間1-5全覆蓋支援
├── 實作狀態：room_dispatch_simple.yaml (已啟用)
└── Work ID: 220001 (kuka-移動貨架)
```

#### 🟢 第4級：空料架搬運 (Priority: 40) - 雙路徑流程
```
房間入口空 Rack 處理（兩個獨立流程）

**流程A：空料架到房間出口** (優先) ❌ 待實作
房間X入口 (空Rack) → 房間X出口
├── 觸發條件：A/B面都為空（0個載具），房間出口位置空閒
├── 用途：直接準備收集處理完成的Carrier，優化流程效率
├── 執行間隔：建議 15 秒
├── 流程：empty_rack_inlet_to_outlet.yaml (待開發)
└── Work ID: 220001 (kuka-移動貨架)

**流程B：空料架到系統空車停放區** (備選) ❌ 待實作
房間X入口 (空Rack) → 系統空車停放區 (31-34)
├── 觸發條件：A/B面都為空（0個載具），房間出口位置已佔用
├── 用途：暫存空Rack避免出口堵塞，等待後續調度使用
├── 執行間隔：建議 20 秒
├── 流程：empty_rack_inlet_to_parking.yaml (待開發)
└── Work ID: 220001 (kuka-移動貨架)

**防衝突機制**：
- 兩個流程都檢查是否已存在未完成任務
- 透過執行間隔差異實現優先級（流程A更頻繁）
- 房間1-5全覆蓋支援
```

#### ~~第3級：人工收料區搬運~~ 🛑 **已確認移除**
```
原規劃：人工收料區內的 Rack 重新分配和最佳化

移除原因：
- 人工收料區 (21-22) 後續全由人工處理
- 不需要 AGVC WCS 或 RCS 進行 AGV 搬運
- 送到人工收料區後，透過 OPUI-HMI「移出系統」即可
```
...
新流程：
1. 人工收料完成 → OPUI-HMI「移出系統」
2. location_id = null → 脫離系統追蹤
3. 人工搬運 → 到系統外倉儲空間
4. 需要時 → 人工搬運到射出機旁
5. OPUI「加入料架」→ 輸入 Rack 編號（如 "101"）
6. 系統更新 → 重新進入系統循環使用

舊流程（已停用）：
人工回收空料架區 (91-92) → 空料架回收區 (51-54)
Work ID: 230001 (kuka-流程觸發) - 不再使用
```
...
```
Rack 生命週期流程（手動管理模式）
1. 系統外倉儲 (location_id = null) → 實體存在但系統不追蹤
2. OPUI 加入料架 → 進入系統準備區 (11-18)
3. 房間製程 → 實際的泡藥水處理（OCR NG 即時處理）
4. 人工收料區 (21-22) → 製程完成後的人工處理
...
- **房間動態擴展**: 支援房間1-5的靈活配置
- **手動管理整合**: OPUI-HMI 和 OPUI 管理 Rack 生命週期
...
### 人工收料區完整回收流程
**背景**：目前人工收料區（21-22）的 rack 回收流程不完整，rack 無法真正離開系統管理。
...
階段 1：滿 rack 到人工收料區
1. 製程完成的滿 rack
2. KUKA AGV 搬運到人工收料區（Location ID: 21-22）
