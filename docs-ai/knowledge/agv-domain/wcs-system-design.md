...
## 🎯 適用場景
- 理解 RosAGV 中以 TAFL WCS 為核心的任務自動化系統架構
- 掌握 TAFL (Task Automation Flow Language) 的執行引擎和流程調度邏輯
- 了解 TAFL Editor 視覺化設計與 TAFL WCS 的整合關係

## 📋 系統概述

**WCS (Warehouse Control System)** 是 RosAGV 系統的核心決策引擎，以 **TAFL WCS** 為統一實作核心，負責管理 Rack 車的搬移需求決策和任務調度。

### 🎯 TAFL WCS 核心定位
- **流程自動化**: 基於 TAFL 語言的任務自動化執行引擎
- **視覺化設計**: TAFL Editor 提供拖拽式流程設計介面
- **靈活調度**: 支援複雜的條件判斷、迴圈和多分支邏輯
- **生產就緒**: 已完全實作並投入生產使用
...
### 🔬 TAFL WCS (tafl_wcs_ws) - 新一代 WCS 系統
**核心職責**:
- **TAFL 執行引擎**: 解析和執行 TAFL 流程定義
- **任務自動化**: 基於 TAFL 語言的複雜流程自動化
- **資料庫整合**: 與 db_proxy_ws 無縫整合，執行資料庫操作
- **ROS 2 節點架構**: 提供完整的系統整合
...
## 🔗 交叉引用
- **TAFL WCS**: `app/tafl_wcs_ws/` - 新一代 TAFL 基礎 WCS 系統
- **TAFL Editor**: AGVCUI 中的 `/tafl/editor` - 視覺化流程編輯器
