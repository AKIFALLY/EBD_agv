...
### 階段1：上料階段（已更新為手動管理模式）
1. **手動Rack管理** → 作業員從倉儲區手動搬運空Rack到作業區
   - 使用OPUI「加入Rack」功能將Rack納入系統管理
   - 系統分配location_id到對應停車格
2. **作業區上貨** → 作業員將貨物放上Rack
   - 停車格位置記錄在 `machine.parking_space_1/2` 欄位
3. **OPUI操作** → 作業員透過OPUI設定產品資訊，產生`status=0`的task
...
11. **人工收料** → 作業員從人工收料區取走完成品
    - 作業員使用OPUI-HMI「移出系統」功能
    - 系統設置rack.location_id = null
    - 作業員手動將空Rack搬到倉儲區存放
12. **循環使用** → 需要時從倉儲區取出
    - 手動搬運到射出機
    - 透過OPUI「加入Rack」重新納入系統管理
...
- **Simple WCS 開發**: @docs-ai/operations/development/flow-wcs-development.md - 開發指導
