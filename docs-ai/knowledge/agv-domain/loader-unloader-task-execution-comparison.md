...
> ⚠️ **重要說明：本文檔包含歷史內容**
>
> 本文檔原包含大量 TAFL WCS 配置範例，TAFL WCS 已於 2025 年棄用。
>
> **當前系統**：RosAGV 已改用 **KUKA WCS** 作為倉儲控制系統。
>
> 本文檔保留僅供：
> 1. 理解 Loader 和 Unloader AGV 的設計差異（仍然適用）
> 2. 機械臂能力和批量處理策略（仍然適用）
> 3. TAFL 流程配置範例（歷史參考）
>
> 現行系統請參考：
...
## 🎯 適用場景
- 理解 Loader 和 Unloader AGV 的任務執行差異（仍然適用）
- ~~設計和調試 TAFL 流程時選擇正確的 batch_size~~（⚠️ 歷史內容：TAFL 已棄用，當前使用 KUKA WCS）
- 優化任務調度策略（仍然適用）
- 排查任務執行問題（仍然適用）
- 為新車型設計提供參考（仍然適用）
...
> ⚠️ **歷史範例提示**
>
> 以下 TAFL 配置範例為舊版倉儲控制系統配置，僅供歷史參考。
>
> **當前系統**: RosAGV 已改用 KUKA WCS 作為倉儲控制系統。

#### TAFL 任務配置範例 - 標準設備（歷史參考）
```yaml
# loader_take_boxin_transfer.yaml（入口箱）- 舊版 TAFL 範例
stations:
  - station: 1
    work_id: 2010101
