        ...
        if self.carrier_query_sended and self.carrier_query_success and self.selected_port_pair:
            ...
            if self.carrier_id_min is not None or self.carrier_id_max is not None:
                ...
                if validation_passed:
                    ...
                    # 設定 context 變數
                    context.get_unloader_agv_port_back = self.agv_port_number
                    context.get_take_quantity = take_quantity
                    self.node.get_logger().info(f"設定 take_quantity = {take_quantity}")

                    self.node.get_logger().info(f"AGV {self.selected_pair_name} 檢查完成，進入下一個狀態")
