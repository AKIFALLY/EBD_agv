...
class PutAgvState(BaseRobotState):
    ...
    def enter(self):
        self.node.get_logger().info("Loader Robot Take Soaker 目前狀態: PutAgv")
        self.update_carrier_success = False
        self._reset_state()

    def leave(self):
        self.node.get_logger().info("Loader Robot Take Soaker 離開 PutAgv 狀態")
        self.update_carrier_success = False
        self._reset_state()
    ...
    def update_carrier_database(self, context: RobotContext):
        ...
        self.node.get_logger().info(
            f"🔄 開始更新 Carrier: {context.carrier_id} 最終位置到 AGV 端口 {carrier.port_id}")
    ...
    def handle(self, context: RobotContext):
        self.node.get_logger().info("Loader Robot Take Soaker PutAgv 狀態")
    ...
    def _execute_robot_logic(self, context: RobotContext, PUT_LOADER_AGV_PGNO, read_pgno):
        ...
        match self.step:
            case RobotContext.IDLE:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV IDLE")
                self.step = RobotContext.CHECK_IDLE
            case RobotContext.CHECK_IDLE:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV CHECK_IDLE")
            ...
            case RobotContext.WRITE_CHG_PARA:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV WRITE CHG PARA")
                if not self.sent:
            ...
            case RobotContext.CHECK_CHG_PARA:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV CHECK CHG PARA")
            ...
            case RobotContext.WRITE_PGNO:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV WRITE_PGNO")
                if not self.sent:
            ...
            case RobotContext.CHECK_PGNO:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV CHECK_PGNO")
                if read_pgno is None:
            ...
            case RobotContext.ACTING:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV ACTING")
                if read_pgno is None:
                    self.node.get_logger().debug("⏳等待讀取PGNO回應...")
                    return
                if read_pgno.value == (PUT_LOADER_AGV_PGNO):
            ...
            case RobotContext.FINISH:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV Finish")
                if read_pgno is None:
                    self.node.get_logger().info("⏳等待讀取PGNO回應...")
                    return
                if read_pgno.value == Robot.IDLE:
            ...
            case RobotContext.UPDATE_DATABASE:
                self.node.get_logger().info("Loader Robot Take Soaker PUT LOADER AGV UPDATE_DATABASE")
                if not self.sent:
                    ...
                    # 移除循環邏輯：put_agv_state.py 完成後直接轉換到 CompleteState 結束流程
                    self.node.get_logger().info("✅ Take Soaker 完成: 進入 CompleteState")
