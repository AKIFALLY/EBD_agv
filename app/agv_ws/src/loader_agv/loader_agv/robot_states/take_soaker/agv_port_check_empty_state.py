...
class AgvPortCheckEmptyState(State):
    ...
    def __init__(self, node: Node):
        ...
        # AGV 端口選擇表格：檢查哪些端口是空的（適合放入從浸泡設備取出的貨物）
        # 0 = 空的端口，1 = 有貨的端口
        # 優先選擇編號較小的空端口
    ...
    def enter(self):
        self.node.get_logger().info("Loader Robot Take Soaker 目前狀態: AgvPortCheckEmpty")
        self._reset_state()

    def leave(self):
        self.node.get_logger().info("Loader Robot Take Soaker 離開 AgvPortCheckEmpty 狀態")
        self._reset_state()
    ...
    def _handle_port_selection(self, context: RobotContext):
        ...
        if self.select_agv_port in port_messages:
            ...
            self.node.get_logger().info(
                f"Loader Robot Take Soaker AgvPortCheckEmpty 狀態: {desc}")
            ...
            self.node.get_logger().info("Loader Robot Take Soaker AgvPortCheckEmpty 狀態: AGV端口已滿")
        ...
        if self.check_ok and self.carrier_query_success:
            ...
            if 1 <= selected_port_number <= 4:
                ...
                if self.earliest_carrier is None and port_eqp_empty:
                    ...
                    self.node.get_logger().info("AGV端口檢查完成，準備執行取浸泡操作")

                    # 轉換到下一個狀態 - TakeSoakerState
                    self.node.get_logger().info("✅ AGV 端口檢查完成: 進入 TakeSoakerState")
