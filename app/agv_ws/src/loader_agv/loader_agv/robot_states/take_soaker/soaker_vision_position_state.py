...
class SoakerVisionPositionState(BaseVisionPositionState):
    ...
    def enter(self):
        self.node.get_logger().info(
            "[Station-based 1格] Loader Robot Take Soaker 目前狀態: SoakerVisionPosition")
        self._reset_state()

    def leave(self):
        self.node.get_logger().info(
            "[Station-based 1格] Loader Robot Take Soaker 離開 SoakerVisionPosition 狀態")
        self._reset_state()

    def handle(self, context: RobotContext):
        self.node.get_logger().info(
            f"[Station-based 1格] Loader Robot Take Soaker 泡藥機視覺定位中 "
            f"(Work ID {context.work_id})")

        # 視覺定位完成後進入 SoakerCheckHaveState（檢查泡藥機是否有貨）
