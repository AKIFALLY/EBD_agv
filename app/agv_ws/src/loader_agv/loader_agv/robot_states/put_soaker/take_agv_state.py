    ...
    def _execute_robot_logic(self, context: RobotContext, TAKE_LOADER_AGV_PGNO, read_pgno):
        ...
        match self.step:
            ...
            case RobotContext.CHECK_CHG_PARA:
                self.node.get_logger().info("Loader Robot Put Soaker TAKE LOADER AGV CHECK CHG PARA")
            ...
            case RobotContext.WRITE_PGNO:
                self.node.get_logger().info("Loader Robot Put Soaker TAKE LOADER AGV WRITE_PGNO")
                if not self.sent:
            ...
            case RobotContext.CHECK_PGNO:
                self.node.get_logger().info("Loader Robot Put Soaker TAKE LOADER AGV CHECK_PGNO")
                if read_pgno is None:
            ...
            case RobotContext.ACTING:
                self.node.get_logger().info("Loader Robot Put Soaker TAKE LOADER AGV ACTING")
                if read_pgno is None:
                    self.node.get_logger().info("⏳等待讀取PGNO回應...")
                    return
                if read_pgno.value == (TAKE_LOADER_AGV_PGNO):
