...
class PreDryerVisionPositionState(BaseVisionPositionState):
    ...
    def enter(self):
        self.node.get_logger().info(
            "[Station-based 2格] Robot Put PreDryer 目前狀態: PreDryerVisionPosition")
        self._reset_state()

    def leave(self):
        self.node.get_logger().info(
            "[Station-based 2格] Robot Put PreDryer 離開 PreDryerVisionPosition 狀態")
        self._reset_state()

    def handle(self, context: RobotContext):
        self.node.get_logger().info(
            f"[Station-based 2格] Robot Put PreDryer 預烘台視覺定位中 "
