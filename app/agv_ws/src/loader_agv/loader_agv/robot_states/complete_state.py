...
class CompleteState(State):
    ...
    def handle(self, context: RobotContext):
        ...
        # 清理相關的狀態變數
        if hasattr(context, 'take_transfer_continue'):
            context.take_transfer_continue = False
        if hasattr(context, 'carrier_id'):
            context.carrier_id = 0  # 重置 carrier_id (loader_agv 使用單一值)
        if hasattr(context, 'get_transfer_port'):
            context.get_transfer_port = 1  # 重置為預設值
        ...
        # 重置 BOXIN port 狀態
        if hasattr(context, 'boxin_port1'):
            context.boxin_port1 = False
        if hasattr(context, 'boxin_port2'):
            context.boxin_port2 = False
        if hasattr(context, 'boxin_port3'):
            context.boxin_port3 = False
        if hasattr(context, 'boxin_port4'):
            context.boxin_port4 = False

        # 重置其他相關變數
        if hasattr(context, 'get_loader_agv_port_front'):
            context.get_loader_agv_port_front = 1
        if hasattr(context, 'get_loader_agv_port_side'):
