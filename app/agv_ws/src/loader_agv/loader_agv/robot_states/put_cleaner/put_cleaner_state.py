...
class PutCleanerState(BaseRobotState):
    ...
    def update_carrier_database(self, context: RobotContext):
        ...
        self.node.get_logger().info(
            f"🔄 開始更新 Carrier: {context.carrier_id} 到清洗機端口 {carrier.port_id}")
    ...
    def handle(self, context: RobotContext):
        self.node.get_logger().info("Loader Robot Put Cleaner PutCleaner 狀態")
    ...
    def _execute_robot_logic(self, context: RobotContext, PUT_CLEANER_PGNO, read_pgno):
        ...
        match self.step:
            ...
            case RobotContext.FINISH:
                self.node.get_logger().info("Loader Robot Put Cleaner PUT CLEANER Finish")
                if read_pgno is None:
                    self.node.get_logger().info("⏳等待讀取PGNO回應...")
                    return
                if read_pgno.value == Robot.IDLE:
                    self.node.get_logger().info("✅放清洗機完成")
                    self.step = RobotContext.UPDATE_DATABASE
                else:
                    self.node.get_logger().info("❌放清洗機失敗")
            case RobotContext.UPDATE_DATABASE:
                self.node.get_logger().info("Loader Robot Put Cleaner PUT CLEANER UPDATE_DATABASE")
                if not self.sent:
                    ...
                    # 檢查 put_cleaner_continue 標誌決定下一個狀態
                    if context.put_cleaner_continue:
                        self.node.get_logger().info("✅ Put Cleaner 完成: put_cleaner_continue=True，返回 CleanerCheckHaveState 繼續處理")
                        from loader_agv.robot_states.put_cleaner.cleaner_check_have_state import CleanerCheckHaveState
                        context.set_state(CleanerCheckHaveState(self.node))
                    else:
                        self.node.get_logger().info("✅ Put Cleaner 完成: put_cleaner_continue=False，進入 CompleteState")
