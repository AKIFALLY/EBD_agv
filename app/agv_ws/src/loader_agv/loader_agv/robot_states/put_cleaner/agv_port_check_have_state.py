...
class AgvPortCheckHaveState(State):
    ...
    def enter(self):
        self.node.get_logger().info("Loader Robot Put Cleaner 目前狀態: AgvPortCheckHave")
        self._reset_state()

    def leave(self):
        self.node.get_logger().info("Loader Robot Put Cleaner 離開 AgvPortCheckHave 狀態")
        self._reset_state()
    ...
    def _handle_port_selection(self, context: RobotContext):
        ...
        if has_cargo:
            ...
            self.node.get_logger().info(
                f"Loader Robot Put Cleaner AgvPortCheckHave 狀態: 檢測到 AGV 端口 {cargo_ports} 有貨物")
            self.node.get_logger().info("準備查詢 Carrier 以選擇最適合的端口")
            # 不在此處設定 context.get_loader_agv_port_side，等 carrier 查詢完成後再設定
            self.check_ok = True
        else:
            self.node.get_logger().info("Loader Robot Put Cleaner AgvPortCheckHave 狀態: AGV端口都沒有貨物")
    ...
    def handle(self, context: RobotContext):
        ...
        if self.check_ok and self.carrier_query_success and self.earliest_carrier:
            ...
            if 1 <= selected_port_number <= 4:
                ...
                if port_has_cargo:
                    # 驗證成功：設定 context.get_loader_agv_port_side 並轉換到 CompleteState
