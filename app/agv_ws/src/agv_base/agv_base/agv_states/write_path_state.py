...
class WritePathState(State):
    ...
    def handle(self, context):
        ...
        # 檢查寫入次數是否超過5次
        if self.count > 5:
            self.node.get_logger().info("✅ 路徑資料寫入失敗過多，寫入異常到PLC")
        ...
        if not self.node.agv_status.AGV_PATH and self.step == 0:
            ...
            try:
                ...
                self.node.get_logger().error(f"❌ 計算路徑失敗- 現在位置: {self.TagNo} ,目標節點: {self.node.node_id}")
            ...
            for i in range(len(self.path)):
                ...
                for tag in self.source_data:
                    ...
                    if tag.get('TagNo') == x:
                        ...
                        self.dataValue[i*20+4] ,self.dataValue[i*20+5] = self.split_32_to_16(tag.get('Tag_X')) # Tag_X_Index=4
                        self.dataValue[i*20+9] ,self.dataValue[i*20+10] = self.split_32_to_16(tag.get('Tag_Y'))# Tag_Y_Index=9
                        ...
                        if len(self.safe_sensor_setting) >= 1:
                            self.dataValue[i*20+6] =  self.safe_sensor_setting[0] #SafeSensorSetting_Index=6
                        ...
                        if len(self.shift) >= 3:
                            self.dataValue[i*20+14] ,self.dataValue[i*20+15] =  self.split_32_to_16(self.shift[2])  #旋轉角度
                        break
            ...
            # 將路徑資料寫入PLC
            self.plc_client.async_write_continuous_data('DM', '3000', string_values_1, self.write_path_callback)     #PLC寫入路徑
            self.plc_client.async_write_continuous_data('DM', '4000', string_values_2, self.write_path_callback)
