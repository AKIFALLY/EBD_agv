...
class BaseRobotState(State):
    ...
    def _set_hokuyo_busy_entrance(self):
        """設定 Entrance 流程的 Hokuyo write_busy = 1"""
    ...
    def _set_hokuyo_busy_exit(self):
        """設定 Exit 流程的 Hokuyo write_busy = 1"""
        if not self.hokuyo_busy_write_completed:
            try:
                self.node.hokuyo_dms_8bit_2.write_busy("1")
                self.node.get_logger().info("✅ Hokuyo_2 write_busy=1 設定完成")
                self.hokuyo_busy_write_completed = True
            except Exception as e:
                self.node.get_logger().error(f"❌ Hokuyo write_busy 設定失敗: {e}")
                # 即使失敗也標記為完成，避免無限重試
    ...
    def _handle_hokuyo_input_entrance(self):
        """統一處理 Entrance 流程的 Hokuyo Input 更新 - 頻率控制版本（每秒一次）"""
        ...
        if current_time - self.last_hokuyo_input_update_time >= self.hokuyo_input_update_interval:
            ...
            self.node.get_logger().debug(
                f"Hokuyo Input 更新執行 (entrance) - 間隔: {current_time - self.last_hokuyo_input_update_time:.2f}s")
    ...
    def _handle_hokuyo_input_exit(self):
        """統一處理 Exit 流程的 Hokuyo Input 更新 - 頻率控制版本（每秒一次）"""
        hokuyo = self.node.hokuyo_dms_8bit_2
        current_time = time.time()
        ...
        if current_time - self.last_hokuyo_input_update_time >= self.hokuyo_input_update_interval:
            ...
            self.last_hokuyo_input_update_time = current_time
            self.node.get_logger().debug(
                f"Hokuyo Input 更新執行 (exit) - 間隔: {current_time - self.last_hokuyo_input_update_time:.2f}s")
