...
class WaitRotationState(State):
    ...
    def enter(self):
        self.node.get_logger().info("Robot Exit 目前狀態: WaitRotation")
        self.rack_response_ok = False
        self.sent = False

    def leave(self):
        self.node.get_logger().info("Robot Exit 離開 WaitRotation 狀態")
        self.rack_response_ok = False
        self.sent = False
    ...
    def handle(self, context: RobotContext):
        self.node.get_logger().info("Robot Exit WaitRotation 狀態")

        match self.step:
            case 0:
                # Case 4 from check_rack_side_state.py - 等待旋轉狀態
            ...
            case 1:
                # Case 5 from check_rack_side_state.py - 完成檢查
