...
此工作空間作為系統的核心，整合了 PLC 通訊、感測器處理、路徑規劃和任務執行等關鍵功能。它採用雙層狀態管理架構，包含 AGV 層狀態和通用狀態，支援複雜的任務流程和狀態轉換。系統具備完整的記憶體管理（65536 words）、50ms 主迴圈控制和 1.5 秒狀態發佈週期。
...
#### Cargo Mover AGV ✅ (完整實作)
- **功能**: 用於貨物搬運的 AGV，負責房間門口的 Rack ↔ 傳送箱轉移作業
- **啟動狀態**: 自動啟動 (`ros2 launch cargo_mover_agv launch.py`)
- **實作程度**: 100% - 完整的狀態機、機器人控制、視覺定位、OCR 識別
- **特色**: Hokuyo 8bit 光通訊模組整合、SensorPart 3D 視覺定位、OCR 產品識別、製程適配性檢查
- **業務流程**: 3D定位 → OCR識別 → 製程匹配 → Carrier抓取 → 傳送箱放料 → A/B雙面處理

#### Unloader AGV ✅ (完整實作)
- **功能**: 專門用於卸載作業的 AGV，負責後段製程的批量卸載操作
- **啟動狀態**: 自動啟動 (`ros2 launch unloader_agv launch.py`)
- **實作程度**: 100% - 完整的狀態機、機器人控制、批量處理邏輯
- **特色**: 一次2格批量操作、預烘機/烘箱/出口傳送箱整合、4種核心業務流程
- **業務流程**: 從預烘機取料 → 放入烘箱上排 → 從烘箱下排取料 → 放入出口傳送箱
