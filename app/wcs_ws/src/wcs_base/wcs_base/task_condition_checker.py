        ...
        try:
            ...
            while iteration_count < self.max_iterations:
                ...
                if not condition_result:
                    ...
                    self.logger.warning(f"⚠️ 無法取得 ID {current_id} 的條件資料 (連續失敗 {consecutive_failures}/{max_consecutive_failures})")
                    if consecutive_failures >= max_consecutive_failures:
                        self.logger.error(f"❌ 連續失敗次數過多，結束檢查")
                ...
                if not parse_result:
                    consecutive_failures += 1
                    self.logger.warning(f"⚠️ ID {current_id} 解析失敗 (連續失敗 {consecutive_failures}/{max_consecutive_failures})")
                    if consecutive_failures >= max_consecutive_failures:
                        self.logger.error(f"❌ 連續失敗次數過多，結束檢查")
                        return False, collected_data
                    current_id = start_id
                ...
                if not success:
                    consecutive_failures += 1
                    self.logger.info(f"📋 ID {current_id} 查詢未成功 (連續失敗 {consecutive_failures}/{max_consecutive_failures})")
                    if consecutive_failures >= max_consecutive_failures:
                        self.logger.error(f"❌ 連續失敗次數過多，結束檢查")
                        return False, collected_data
                    current_id = start_id
                ...
                if next_id:
                    ...
                    if next_id_result == -1:
                        ...
                        # next_id 處理失敗，結束檢查
                        self.logger.info(f"📋 next_id '{next_id}' 處理失敗，結束檢查")
                        return False, collected_data
                else:
                    # 沒有 next_id，檢查是否應該結束
                    if data_list:
                        ...
                        if all_false_no_next:
                            ...
                            # 有部分條件為True但無next_id，或條件結果不明確，結束檢查
                            self.logger.info(f"📋 ID {current_id} 條件處理完成但無明確next_id，結束檢查")
                            return False, collected_data
                    else:
                        # data_list為空，表示查詢無結果，結束檢查
                        self.logger.info(f"📋 ID {current_id} 查詢無結果，結束檢查")
                        return False, collected_data
