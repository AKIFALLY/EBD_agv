...
import unittest
import rclpy
from alan_room_task_build.room_task_build_node import RoomTaskBuildNode


class TestRoomTaskBuild(unittest.TestCase):
    """Room Task Build 測試類別"""
    ...
    def test_agv_name_generation(self):
        """測試 AGV 名稱生成邏輯"""
        # 測試案例 1: room_id=2, agv_type="LOADER"
        agv_type = "LOADER"
        room_id = 2
        expected_name = "loader02"
        actual_name = f"{agv_type.lower()}{room_id:02d}"
        self.assertEqual(actual_name, expected_name)

        # 測試案例 2: room_id=2, agv_type="UNLOADER"
        agv_type = "UNLOADER"
        room_id = 2
        expected_name = "unloader02"
        actual_name = f"{agv_type.lower()}{room_id:02d}"
        self.assertEqual(actual_name, expected_name)

        # 測試案例 3: room_id=1, agv_type="LOADER"
        agv_type = "LOADER"
        room_id = 1
        expected_name = "loader01"
        actual_name = f"{agv_type.lower()}{room_id:02d}"
        self.assertEqual(actual_name, expected_name)

        # 測試案例 4: room_id=10, agv_type="UNLOADER"
        agv_type = "UNLOADER"
        room_id = 10
        expected_name = "unloader10"
        actual_name = f"{agv_type.lower()}{room_id:02d}"
        self.assertEqual(actual_name, expected_name)
