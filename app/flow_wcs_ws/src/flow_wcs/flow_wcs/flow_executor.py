...
class FlowExecutor:
    ...
    def register_functions(self) -> Dict:
        ...
        return {
            ...
            'control.switch': self.switch_advanced,  # Advanced switch with conditions
    ...
    def execute_function_test(cls, function_name: str, params: Dict, variables: Dict = None) -> Any:
        ...
        try:
            ...
            if inspect.iscoroutinefunction(bound_handler):
                ...
                result = asyncio.run(bound_handler(params))
            else:
                result = bound_handler(params)
    ...
    async def execute(self) -> Dict:
        ...
        try:
            ...
            # Log flow execution to database
    ...
    async def execute_step(self, step: Dict):
        ...
        if 'skip_if' in step:
            ...
            if step_id in ['check_rack_needs_rotation', 'check_a_side_complete', 'create_rotation_task', 'log_task_created']:
                self.log_info(f"Step {step_id} skip_if debug:")
                ...
                self.log_info(f"Step {step_id} skip_if condition: {condition} = {result}")
            if result:
    ...
    async def execute_function(self, step: Dict):
        ...
        resolved_params = await self.resolve_variables(params)
        ...
        if exec_cmd in self.functions:
            ...
            self.log_info(f"========================================")
            self.log_info(f"📍 Executing function: {exec_cmd}")
    ...
    def switch_advanced(self, params: Dict) -> Any:
        """Advanced switch control with condition evaluation"""
        value = params.get('value')
        cases = params.get('cases', [])
