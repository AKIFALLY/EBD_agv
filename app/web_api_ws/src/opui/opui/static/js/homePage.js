...
export const homePage = (() => {
    ...
    function updateNumBtnClass(op) {
        ["left", "right"].forEach(side => {
            const sideData = op[side];
            if (sideData?.product instanceof Array) {
                const idx = sideData.productSelected ?? 0;
                const product = sideData.product[idx];
                const size = (product.size || '').toUpperCase();
                const count = typeof product.count === "number" ? product.count : 0;

                document.querySelectorAll(`.matrix-space[data-side='${side}']`).forEach(matrix => {
                    const btnSize = matrix.dataset.size;
                    const btnNum = Number(matrix.dataset.num);
                    const btn = matrix.querySelector('button');

                    matrix.classList.toggle('hidden', btnSize !== size);

                    if (btnSize === size && btnNum <= count) {
    ...
    function updateDispatchFullButton() {
        const machineId = clientStore.getState().machineId;
        const machines = machinesStore.getState().machines || [];
        const machine = machines.find(m => Number(m.id) === Number(machineId));
        ["left", "right"].forEach(side => {
    ...
    function handleMachinesChange(newState) {
        updateCallEmptyButton();
        updateDispatchFullButton();
    }
