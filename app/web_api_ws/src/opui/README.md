# OPUI - 操作員介面系統

OPUI (Operator User Interface) 是一個基於 ROS2、FastAPI + Socket.IO 的即時 Web 應用程式，專為 AGV 調度系統的操作員介面設計。

## 🎯 系統概述

OPUI 負責提供操作員友善的 Web 介面，用於管理 AGV 任務調度、監控系統狀態，並與後端 RCS 系統協作完成自動化倉儲作業。

### 核心功能
- 🚛 **AGV 任務管理**：加入料架（add rack）和派車（dispatch car）操作
- 📊 **即時監控**：任務狀態、料架位置、機台狀態即時更新
- 🏭 **多機台支援**：支援多個生產機台的並行操作
- 📦 **料架管理**：料架分配、移動追蹤、狀態同步
- 🔄 **資料同步**：與資料庫和其他系統的即時資料同步

### 系統特色
- ✅ **模組化前端架構**：按頁面功能分離的 JavaScript 架構
  - `index.js`: 共用功能（全域初始化、Store 狀態管理、Socket 連線處理）
  - `pages/homePage.js`: Home 頁面專用功能（產品選擇、數量設定、房號選擇）
  - `pages/settingPage.js`: Settings 頁面專用功能（產品管理、系統設定）
  - `pages/rackPage.js`: Rack 頁面專用功能（料架管理）
- ✅ **分離式後端架構**：按功能職責分離的 Python 架構
  - `op_ui_server.py`: FastAPI 伺服器和 HTTP 路由處理
  - `op_ui_socket.py`: Socket.IO 事件處理和即時通訊功能
- ✅ **統一狀態管理**：基於 miniStore 的輕量級狀態管理
  - `userStore`: 用戶狀態管理（clientId、machineId、連線狀態）
  - `operationStore`: 操作狀態管理（左右側操作狀態）
  - `dataStore`: 資料狀態管理（products、machines、rooms、parking）
  - `tasksStore`: 任務狀態管理（活躍任務追蹤）
  - `uiStore`: UI 狀態管理（載入狀態、通知訊息）
- ✅ **直接資料庫連接**：使用 db_proxy 的 connection_pool_manager
- ✅ **即時通訊**：基於 Socket.IO 的雙向即時通訊
- ✅ **新手友善**：適合初學者團隊的簡化架構設計

## 📚 文件導覽

### 核心文檔
- **[架構文件](docs/ARCHITECTURE.md)** - 詳細的系統架構說明
- **[前端架構](docs/FRONTEND_ARCHITECTURE.md)** - 前端模組化架構說明
- **[Socket API 格式指南](docs/SOCKET_API_FORMAT_GUIDE.md)** - API 資料格式說明
- **[維護指南](docs/MAINTENANCE.md)** - 日常維護和故障排除
- **[測試文件](tests/README.md)** - 測試框架和測試指南

### 歷史記錄
- **[歷史修復記錄](docs/archives/)** - 重要的修復和變更記錄
  - 扁平化格式遷移總結
  - Home 頁面顯示修復
  - 效能優化記錄
  - 任務狀態重構記錄

## 🚀 快速開始

```bash
# 1. 建置專案
cd /app/web_api_ws
colcon build --packages-select opui --symlink-install

# 2. 設定環境
all_source  # 或 source install/setup.bash

# 3. 啟動服務
ros2 run opui op_ui_server

# 4. 訪問介面
# 主頁面: http://localhost:8002/
# 設定頁面: http://localhost:8002/setting
# 料架管理頁面: http://localhost:8002/rack
```

## 🏗️ 系統架構

### 整體架構圖

```
┌─────────────────────┐   Socket.IO   ┌─────────────────────┐   db_proxy   ┌─────────────────────┐
│    前端 (Web)       │ ←───────────→ │    後端 (Python)    │ ←──────────→ │   資料庫 (PostgreSQL) │
│                     │               │                     │              │                     │
│ 模組化架構:         │               │ 分離式架構:         │              │ • Client            │
│ • index.js (共用)   │               │ • op_ui_server.py   │              │ • Product           │
│ • pages/            │               │   (API和路由處理)   │              │ • Machine           │
│   ├─homePage.js     │               │ • op_ui_socket.py   │              │ • Room              │
│   ├─settingPage.js  │               │   (Socket.IO功能)   │              │ • Rack              │
│   └─rackPage.js     │               │ • task_monitor.py   │              │ • Task              │
│                     │               │ • database/ops.py   │              │                     │
│ 狀態管理 (miniStore):│               │ • api/ routers      │              │                     │
│ • userStore         │               │ • constants/        │              │                     │
│ • operationStore    │               │                     │              │                     │
│ • dataStore         │               │                     │              │                     │
│ • tasksStore        │               │                     │              │                     │
│ • uiStore           │               │                     │              │                     │
└─────────────────────┘               └─────────────────────┘              └─────────────────────┘
```

### 與其他系統的整合

```
┌─────────────┐    任務創建    ┌─────────────┐    AGV調度    ┌─────────────┐
│    OPUI     │ ────────────→ │     RCS     │ ────────────→ │ KUKA Fleet  │
│  操作員介面  │               │   調度核心   │               │   Adapter   │
└─────────────┘               └─────────────┘               └─────────────┘
       │                             │                             │
       │ 資料庫查詢                   │ 任務監控                     │ AGV控制
       ↓                             ↓                             ↓
┌─────────────┐               ┌─────────────┐               ┌─────────────┐
│  db_proxy   │               │     WCS     │               │    KUKA     │
│  資料庫代理  │               │   倉儲控制   │               │   AGV車隊    │
└─────────────┘               └─────────────┘               └─────────────┘
```

## 📁 專案結構

```
opui/
├── opui/                        # 主要程式碼目錄
│   ├── __init__.py
│   ├── core/                    # 核心服務層
│   │   ├── op_ui_server.py     # FastAPI 伺服器主程式 (API和路由處理)
│   │   ├── op_ui_socket.py     # Socket.IO 事件處理 (即時通訊功能)
│   │   ├── task_service.py     # 任務業務邏輯服務
│   │   └── device_auth.py      # 設備授權驗證
│   ├── database/               # 資料庫層
│   │   └── operations.py       # 資料庫操作 (使用 db_proxy)
│   ├── api/                    # REST API 路由
│   │   ├── _client.py          # 客戶端相關 API
│   │   ├── agv.py              # AGV 相關 API
│   │   ├── license.py          # 授權相關 API
│   │   ├── node.py             # 節點相關 API
│   │   ├── process_settings.py # 製程設定 API
│   │   └── product.py          # 產品相關 API
│   ├── constants/              # 常數定義
│   │   ├── parking_status.py   # 停車位狀態常數
│   │   └── task_status.py      # 任務狀態常數
│   ├── monitoring/             # 監控模組
│   │   └── task_monitor.py     # 任務狀態監控
│   └── frontend/               # 前端資源
│       ├── static/
│       │   ├── index.js        # 共用功能 (全域初始化、Store狀態管理、Socket連線)
│       │   ├── js/
│       │   │   ├── api.js      # Socket.IO 通訊層
│       │   │   ├── store.js    # 統一狀態管理 (appStore)
│       │   │   ├── notify.js   # 通知系統
│       │   │   ├── pages/      # 頁面專用功能模組
│       │   │   │   ├── homePage.js    # Home 頁面專用功能
│       │   │   │   ├── settingPage.js # Settings 頁面專用功能
│       │   │   │   └── rackPage.js    # Rack 頁面專用功能
│       │   │   ├── lib/
│       │   │   │   ├── miniStore.js   # 輕量級狀態管理庫
│       │   │   │   └── socket.io.js   # Socket.IO 客戶端庫
│       │   │   └── performance-test.js # 效能測試工具
│       │   ├── css/            # 樣式檔案
│       │   │   ├── bulma_1_0_4.min.css        # Bulma CSS 框架
│       │   │   ├── materialdesignicons.min.css # Material Design 圖示
│       │   │   └── opui-bulma-extend.css      # OPUI 自定義樣式擴展
│       │   ├── fonts/          # 字型檔案
│       │   │   └── materialdesignicons-webfont.* # Material Design 圖示字型
│       │   └── favicon.png     # 網站圖示
│       └── templates/          # HTML 模板
│           ├── base.html       # 基礎模板
│           ├── navbar.html     # 導航列模板
│           ├── home.html       # 操作主頁
│           ├── setting.html    # 設定頁面
│           ├── rack.html       # 料架管理頁面
│           └── unauthorized.html # 未授權頁面
├── tests/                      # 測試目錄
│   ├── refactor/              # 重構測試
│   │   ├── test_syntax.py     # 語法驗證測試
│   │   └── test_refactor.py   # 架構重構測試
│   ├── validation/            # 功能驗證測試
│   │   ├── test_dispatch_fix.py # 派車功能修復測試
│   │   ├── test_socket_functions.py # Socket.IO 功能測試
│   │   └── test_final_validation.py # 最終驗證測試
│   ├── integration/           # 整合測試
│   │   └── test_integration.py # 系統整合測試
│   ├── run_refactor_tests.py  # 重構測試運行器
│   ├── run_all_tests.py       # 全部測試運行器
│   ├── test_current_architecture.py # 當前架構驗證測試
│   ├── test_db.py             # 資料庫測試
│   ├── test_op_ui_server.py   # 伺服器測試
│   ├── test_op_ui_socket.py   # Socket 事件測試
│   ├── test_performance.py    # 效能測試
│   ├── test_socket_api_format_consistency.py # Socket API 格式一致性測試
│   └── requirements-test.txt  # 測試依賴套件
├── docs/                       # 文件目錄
│   ├── ARCHITECTURE.md         # 架構文件
│   ├── FRONTEND_ARCHITECTURE.md # 前端架構文件
│   ├── SOCKET_API_FORMAT_GUIDE.md # Socket API 格式指南
│   ├── MAINTENANCE.md          # 維護指南
│   └── archives/              # 歷史記錄檔案
├── config/                     # 配置檔案
│   └── settings.py            # 系統設定
├── package.xml                 # ROS2 套件配置
├── setup.py                    # Python 套件配置
├── setup.cfg                   # Python 套件設定
└── README.md                   # 本文件
```

## 🔧 技術棧

### 前端技術
- **JavaScript**: ES6+ 模組系統，無框架 vanilla JS
- **狀態管理**: 自研 miniStore 輕量級狀態管理庫
- **UI 框架**: Bulma CSS 框架
- **圖示**: Material Design Icons
- **即時通訊**: Socket.IO 客戶端

### 後端技術
- **Python**: 3.8+
- **Web 框架**: FastAPI (非同步 Web 框架)
- **即時通訊**: Socket.IO 伺服器
- **資料庫**: PostgreSQL
- **資料庫操作**: db_proxy connection_pool_manager
- **ROS2**: 機器人作業系統第二版

### 基礎設施
- **容器化**: Docker (ROS2 環境)
- **資料庫代理**: db_proxy 統一資料庫連線管理
- **測試框架**: pytest

## 🔧 核心組件

### 1. 後端核心服務 (重構後新架構)

#### op_ui_server.py - FastAPI 伺服器主程式
專責 Web 伺服器和 HTTP 路由管理：

**主要職責**：
- **FastAPI 應用初始化**: 配置 CORS、中間件、靜態檔案
- **HTTP 路由管理**: `/home`, `/setting` 頁面路由
- **API 路由整合**: 整合 process_settings, product, license API
- **Socket.IO 整合**: 將 Socket.IO 整合到 ASGI 應用中
- **設備授權檢查**: 驗證設備 ID 和授權狀態

#### op_ui_socket.py - Socket.IO 事件處理
專責 Socket.IO 事件和即時通訊管理：

**主要事件處理**：
- **連線管理**: `connect`, `disconnect`, `login`, `restore_client_by_id`
- **資料同步**: `client_update`, `notify_products/machines/rooms/parking_list`
- **AGV 操作**: `add_rack` (加入料架), `dispatch_full` (派車)
- **料架管理**: `add_rack`, `del_rack`
- **任務控制**: `cancel_task`, `confirm_delivery`
- **共用邏輯**: 統一的客戶端通知、資料格式化、會話處理

#### task_monitor.py - 任務監控
獨立的任務狀態監控模組：
- 監控進行中的任務狀態變化
- 檢測任務完成條件
- 觸發狀態同步和 UI 更新

#### database/operations.py - 資料庫操作
使用 db_proxy 的 connection_pool_manager 進行資料庫操作：
- 統一的 CRUD 操作介面
- 連線池管理，提高效能
- 支援事務處理

### 2. 前端架構

#### 統一狀態管理 (appStore)
使用單一 Store 管理所有前端狀態：

```javascript
const appStore = createStore('opuiAppState', {
    user: { clientId, machineId, isConnected },      // 使用者狀態
    operation: { left: {...}, right: {...} },        // 操作狀態
    data: { products, machines, rooms, parking },     // 基礎資料
    tasks: { active: {...} },                        // 任務狀態
    ui: { loading, notifications }                    // UI 狀態
});
```

#### 模組化管理器
- **EventManager**: 事件處理管理
- **UIManager**: UI 更新管理
- **PageManager**: 頁面邏輯管理
- **StateManager**: 狀態同步管理

#### 通訊層 (SocketAPI)
統一的前後端通訊介面，封裝所有 Socket.IO 操作

## 🔄 資料流程與業務邏輯

### 1. 系統初始化流程
```
使用者開啟網頁 → Socket 自動連線 → 觸發 login 事件 →
後端驗證並建立會話 → 推送基礎資料 → 前端更新 appStore → UI 渲染
```

### 2. AGV 任務操作流程

#### 加入料架 (Add Rack) 流程
```
作業員手動搬運空rack到作業區 → 點擊「加入料架」按鈕 →
從下拉選單選擇Rack編號 → Socket 發送 add_rack 事件 →
後端記錄rack到停車格 → 回應前端 → 更新 UI 狀態
```

#### 派車 (Dispatch Car) 流程
```
操作員選擇產品和料架 → 點擊派車 → Socket 發送 dispatch_full 事件 →
後端創建 Task 記錄 → 啟動任務監控 → 回應前端 → 更新機台狀態
```

### 3. 任務監控與完成檢測
```
任務創建 → task_monitor 開始監控 → 定期檢查任務狀態 →
檢測完成條件 → 觸發完成回調 → 更新資料庫 → 廣播狀態變更
```

### 4. 即時資料同步
```
資料變更 (料架移動/任務狀態) → 後端檢測 → Socket 廣播事件 →
所有連線客戶端接收 → 更新 appStore → UI 自動重新渲染
```

## 🚛 AGV 任務管理功能

### 加入料架 (Add Rack) 功能
**業務含義**: 記錄作業員手動搬運的空rack到停車格

**操作流程**:
1. **前置條件**: 停車位為空，作業員已手動搬運空rack到作業區
2. **操作步驟**:
   - 操作員點擊「加入料架」按鈕
   - 從彈出的Modal中選擇對應的rack編號
   - 點擊「確認加入」
3. **系統處理**:
   - 系統載入可用rack列表（未在任何停車格的rack）
   - 記錄選擇的rack到對應停車格
   - 更新資料庫中rack位置
   - 廣播停車格狀態變更

**事件參數**:
```python
{
    "side": "left",  # 或 "right"
    "rack": "R001",  # rack編號
    "machine_id": 1,
    "client_id": "client_123"
}
```

### 派車 (Dispatch Car) 功能
**業務含義**: 呼叫 AGV 將停車位上的料架運走

**操作流程**:
1. **前置條件**: 停車位有料架，已選擇產品和數量
2. **操作步驟**: 操作員選擇產品、數量後點擊派車
3. **系統處理**:
   - 收集產品資訊、料架 ID、數量等
   - 創建 dispatch_full 類型任務
   - 啟動任務監控
   - 更新停車位狀態為「派送中」

**任務參數**:
```python
{
    "task_type": "dispatch_full",
    "product_name": "產品A",
    "count": 32,
    "rack_id": 123,
    "room": 2,
    "side": "left"
}
```

### 任務狀態管理

**狀態定義** (與資料庫 13_works_tasks.py 保持同步):
- **status_id = 0**: 請求中 (UI-請求執行任務)
- **status_id = 1**: 待處理 (WCS-任務已接受，待處理)
- **status_id = 2**: 待執行 (RCS-任務已派發，待執行)
- **status_id = 3**: 執行中 (AGV-任務正在執行)
- **status_id = 4**: 已完成 (AGV-任務已完成)
- **status_id = 5**: 取消中 (任務取消)
- **status_id = 51**: WCS-取消中 (WCS-任務取消中，待處理)
- **status_id = 52**: RCS-取消中 (RCS-任務取消中，取消中)
- **status_id = 53**: AGV-取消中 (AGV-取消完成)
- **status_id = 54**: 已取消 (任務已取消)
- **status_id = 6**: 錯誤 (錯誤)

**狀態常數使用**:
為了避免硬編碼，專案中提供了統一的狀態常數定義：

```python
# Python 後端
from opui.constants.task_status import TaskStatus

# 使用常數而非硬編碼數字
if task.status_id == TaskStatus.EXECUTING:
    # 處理執行中的任務
    pass
```

```javascript
// JavaScript 前端
import { TASK_STATUS_ID } from '../constants/taskStatus.js';

// 使用常數而非硬編碼數字
if (status === TASK_STATUS_ID.EXECUTING) {
    // 處理執行中的任務
}
```

**完成檢測邏輯**:
- **派車任務**: 檢測到指定料架離開停車位時完成


## 🗄️ 資料庫整合

### 資料庫架構
使用 `db_proxy` 的 `ConnectionPoolManager` 進行統一資料庫管理：

```python
# 資料庫連線池
db_url_agvc = 'postgresql+psycopg2://agvc:password@192.168.100.254/agvc'
connection_pool = ConnectionPoolManager(db_url_agvc)

# 統一 CRUD 操作介面
client_crud = BaseCRUD(Client, id_column="id")
product_crud = BaseCRUD(Product, id_column="id")
machine_crud = BaseCRUD(Machine, id_column="id")
room_crud = BaseCRUD(Room, id_column="id")
rack_crud = BaseCRUD(Rack, id_column="id")
task_crud = BaseCRUD(Task, id_column="id")
```

### 主要資料表結構

| 資料表 | 用途 | 主要欄位 |
|--------|------|----------|
| **Client** | 客戶端/操作員資訊 | id, clientId, machineId, userAgent |
| **Product** | 產品資料 | id, name, size, room |
| **Machine** | 機台資訊 | id, name, enable, parking_status |
| **Room** | 房間/區域資料 | id, name, description |
| **Rack** | 料架資訊 | id, name, location_id, status |
| **Task** | 任務記錄 | id, name, work_id, status_id, parameters |
| **Work** | 工作類型 | id, name, description |
| **TaskStatus** | 任務狀態 | id, name, description |

### 資料關聯
- **Client** ↔ **Machine**: 操作員與機台的關聯
- **Rack** ↔ **Location**: 料架與停車位的關聯
- **Task** ↔ **Work**: 任務與工作類型的關聯
- **Task** ↔ **TaskStatus**: 任務與狀態的關聯
- **Product** ↔ **Room**: 產品與房間的關聯

## 🚀 安裝與部署

### 環境需求
- **Python**: 3.8+
- **PostgreSQL**: 資料庫伺服器
- **ROS2**: 機器人作業系統 (Docker 容器環境)
- **Node.js**: (可選，用於前端開發工具)

### 安裝步驟

#### 1. 建置 ROS2 套件
```bash
cd /app/web_api_ws
colcon build --packages-select opui --symlink-install
```

#### 2. 設定環境變數
```bash
# 方法一：使用 all_source 腳本 (推薦)
all_source

# 方法二：手動設定
source install/setup.bash
```

#### 3. 啟動服務
```bash
ros2 run opui op_ui_server
```

#### 4. 訪問介面
- **主頁面**: http://localhost:8002/
- **設定頁面**: http://localhost:8002/setting
- **料架管理頁面**: http://localhost:8002/rack

### 配置選項

#### 環境變數
```bash
# CORS 設定
export CORS_ALLOWED_ORIGINS="*"  # 或指定特定域名

# 資料庫連線 (在 database/operations.py 中設定)
DB_URL="postgresql+psycopg2://agvc:password@192.168.100.254/agvc"
```

#### 服務埠號
- **Web 服務**: 8002 (預設)
- **Socket.IO**: 與 Web 服務共用埠號

## 📡 API 文檔

### Socket.IO 事件

#### 客戶端 → 伺服器事件

| 事件名稱 | 參數 | 說明 |
|----------|------|------|
| `login` | `{clientId, machineId, userAgent}` | 使用者登入驗證 |
| `client_update` | `{machineId, ...}` | 更新客戶端設定 |
| `add_rack` | `{side, rack}` | 加入料架操作 |
| `dispatch_full` | `{name, count, rackId, room, side}` | 派車操作 |
| `cancel_task` | `{taskId}` | 取消任務 |
| `add_rack` | `{rackName, locationId}` | 新增料架到停車位 |
| `del_rack` | `{rackId}` | 從停車位移除料架 |

#### 伺服器 → 客戶端事件

| 事件名稱 | 資料格式 | 說明 |
|----------|----------|------|
| `product_list` | `{products: [...]}` | 產品列表更新 |
| `machine_list` | `{machines: [...]}` | 機台列表更新 |
| `room_list` | `{rooms: [...]}` | 房間列表更新 |
| `parking_list` | `{parking: {...}}` | 停車位列表更新 |
| `notify_message` | `{message: "..."}` | 成功通知訊息 |
| `error_message` | `{message: "..."}` | 錯誤訊息 |

### HTTP API 端點

| 方法 | 路徑 | 說明 |
|------|------|------|
| `GET` | `/` | 主頁面 |
| `GET` | `/setting` | 設定頁面 |
| `GET` | `/rack` | 料架管理頁面 |
| `POST` | `/api/products` | 產品相關 API |
| `POST` | `/api/process-settings` | 製程設定 API |

### 資料格式範例

#### 任務創建回應
```json
{
    "success": true,
    "message": "任務創建成功",
    "task_id": 123
}
```

#### 停車位資料格式
```json
{
    "parking": {
        "1": {  // machine_id
            "left": {"node_id": 101, "rack": {...}},
            "right": {"node_id": 102, "rack": {...}}
        }
    }
}
```

## 🛠️ 開發指南

### 開發環境設定

#### 1. 開發工作流程
```bash
# 開發時的標準流程
cd /app/web_api_ws

# 修改程式碼後重新建置
colcon build --symlink-install

# 設定環境變數
all_source

# 啟動開發伺服器
ros2 run opui op_ui_server
```

#### 2. 前端開發規範
- **模組系統**: 使用 ES6 import/export
- **狀態管理**: 統一使用 appStore，避免直接操作 DOM
- **程式碼組織**: 按功能模組化，使用管理器模式
- **樣式**: 使用 Bulma CSS 框架，避免內聯樣式
- **除錯**: 開發時可取消註解 console.log，正式版本需註解

#### 3. 後端開發規範
- **非同步處理**: 使用 async/await 處理所有 I/O 操作
- **資料庫操作**: 統一透過 db_proxy 的 connection_pool_manager
- **錯誤處理**: 使用 try-catch 包裝所有資料庫操作
- **Socket 事件**: 保持事件處理函數簡潔，複雜邏輯移至服務層
- **日誌記錄**: 重要操作需記錄日誌，便於除錯

### 測試框架

#### 運行測試
```bash
# 在 OPUI 專案目錄下運行所有測試
cd /app/web_api_ws/src/opui
python -m pytest tests/ -v

# 運行特定測試檔案
python -m pytest tests/test_db.py -v

# 運行特定測試類別
python -m pytest tests/test_op_ui_socket.py::TestSocketEvents -v

# 使用 ROS2 測試框架
colcon test --packages-select opui
```

#### 測試覆蓋範圍
- **單元測試**: 資料庫操作、Socket 事件處理
- **整合測試**: 前後端整合、多使用者並發
- **效能測試**: Socket 連線效能、API 回應時間

## 🔗 與其他系統的整合

### RCS (Robot Control System) 整合
- **關係**: OPUI 創建任務，RCS 負責實際的 AGV 調度
- **通訊方式**: 透過共享資料庫 (Task 表) 進行任務傳遞
- **資料流**: OPUI 寫入 Task → RCS 讀取並執行 → 更新任務狀態

### WCS (Warehouse Control System) 整合
- **功能**: 任務完成監控和車輛派遣條件評估
- **必要性**: WCS 組件是必需的，負責監控任務完成狀態
- **監控邏輯**: 檢測料架位置變化，判斷任務是否完成

### KUKA Fleet Adapter 整合
- **角色**: 實際的 AGV 車隊控制介面
- **調度流程**: RCS → KUKA Fleet Adapter → KUKA AGV 車隊
- **資料同步**: 車輛狀態和位置資訊回傳至系統

### db_proxy 整合
- **用途**: 統一的資料庫連線和操作管理
- **優勢**: 連線池管理、事務處理、效能最佳化
- **使用方式**: 所有資料庫操作都透過 db_proxy 進行

## 🏗️ 架構重構 (2024年重大更新)

### 重構概述

為了提高程式碼的可維護性和模組化程度，我們對後端架構進行了重大重構：

**重構前**：
- `server.py` - 包含所有伺服器邏輯
- `socket_handler.py` - 包含所有 Socket.IO 事件處理 (845行)

**重構後**：
- `op_ui_server.py` - 專責 FastAPI 伺服器和路由 (約150行)
- `op_ui_socket.py` - 專責 Socket.IO 事件處理 (約800行)

### 重構成果

#### 1. 架構分離優化
- **職責分離**: 伺服器邏輯與 Socket.IO 邏輯完全分離
- **模組化**: 每個檔案都有明確的單一職責
- **可維護性**: 修改 API 路由不會影響 Socket.IO 功能，反之亦然

#### 2. 程式碼重複減少
- **統一通知方法**: `_send_client_notifications()`
- **統一資料格式化**: `_format_client_data()`
- **統一會話處理**: `_handle_client_session()`
- **重複邏輯消除**: `login` 和 `restore_client_by_id` 共用邏輯

#### 3. 派車功能修復
- **問題**: 前端只傳送部分資料 `{ side, rackId }`
- **修復**: 傳送完整資料 `{ side, productName, count, rackId, room }`
- **向後相容**: 支援 `productName` (新) 和 `name` (舊) 兩種參數格式

#### 4. 測試完整性
- **重構測試**: 語法驗證、架構分離測試
- **功能測試**: Socket.IO 功能完整性測試
- **修復測試**: 派車功能修復驗證
- **整合測試**: 系統整合和初始化測試

### 重構測試

```bash
# 運行所有重構測試
cd /app/web_api_ws/src/opui/tests
python run_refactor_tests.py

# 運行特定測試類型
python refactor/test_syntax.py           # 語法驗證
python validation/test_dispatch_fix.py   # 派車修復
python validation/test_final_validation.py # 最終驗證
```

### 向後相容性

- **ROS2 啟動點**: 已更新為 `op_ui_server:main`
- **API 端點**: 完全相容，無需修改前端
- **Socket.IO 事件**: 完全相容，所有事件名稱保持不變
- **資料格式**: 支援新舊兩種參數格式

### 專案清理 (2024年完成)

- **Legacy 檔案移除**: `server_legacy.py` 和 `socket_handler_legacy.py` 已移除
- **測試檔案整理**: 測試檔案已移動到 `tests/` 目錄的適當子目錄
- **架構驗證**: 新增 `test_current_architecture.py` 快速驗證當前架構
- **文檔同步**: 所有文檔已更新以反映當前專案狀態

## 🔧 故障排除

### 常見問題與解決方案

#### 1. Socket 連線問題
```bash
# 檢查服務是否運行
ps aux | grep op_ui_server

# 檢查埠號是否被佔用
netstat -tulpn | grep 8002

# 重新啟動服務
ros2 run opui op_ui_server
```

#### 2. 資料庫連線錯誤
```bash
# 檢查資料庫服務狀態
systemctl status postgresql

# 測試資料庫連線
psql -h 192.168.100.254 -U agvc -d agvc

# 檢查連線池狀態 (查看日誌)
```

#### 3. ROS2 環境問題
```bash
# 重新建置套件
colcon build --packages-select opui --symlink-install

# 確保環境變數正確設定
all_source

# 檢查套件是否正確安裝
ros2 pkg list | grep opui
```

#### 4. 前端載入問題
- 清除瀏覽器快取 (Ctrl+F5)
- 檢查開發者工具的 Console 錯誤訊息
- 確認靜態檔案路徑正確

## 📚 相關專案

| 專案名稱 | 功能說明 | 關係 |
|----------|----------|------|
| **rcs_core** | AGV 調度核心系統 | 任務執行者 |
| **db_proxy** | 資料庫代理層 | 資料存取層 |
| **wcs_ws** | 倉儲控制系統 | 任務監控組件 |
| **agvcui** | AGV 控制介面 | 車輛控制 |
| **agvui** | AGV 監控介面 | 狀態監控 |

## 📄 授權

本專案為內部開發使用，適用於 AGV 自動化倉儲系統。

---

*最後更新: 2025-07-15*
*版本: 2025 重構版 - 專案結構同步更新*
